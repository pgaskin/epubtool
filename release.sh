...
for GOOS in linux windows darwin; do
    for GOARCH in amd64 386; do
        echo "Building epubtool $APP_VERSION for $GOOS $GOARCH"
        GOOS=$GOOS GOARCH=$GOOARCH go build -ldflags "-X main.version=$APP_VERSION" -o "build/epubtool-$GOOS-$(echo $GOARCH|sed 's/386/32bit/g'|sed 's/amd64/64bit/g')$(echo $GOOS|sed 's/windows/.exe/g'|sed 's/linux//g'|sed 's/darwin//g')"
    done
done
...
if [[ "$SKIP_UPLOAD" != "true" ]]; then
    ...
    GITHUB_TOKEN=$GITHUB_TOKEN github-release release \
        ...
        --repo epubtool \
        --tag $APP_VERSION \
        --name "epubtool $APP_VERSION" \
        --description "$(cat build/release-notes.md)"

    for f in build/epubtool-*;do 
        fn="$(basename $f)"
        echo "Uploading $fn"
